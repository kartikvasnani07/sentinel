...
Fallback1: Coqui VITS (offline neural TTS)
...
class TTSEngine:
    ...
    def _ensure_piper_model(self):
        ...
        # Basic default model download flow if path not provided
        if not self.piper_model_path or not Path(self.piper_model_path).exists():
            # In a real setup, we'd automatically download a model.
            # For now, require the user to provide one or fallback.
            raise RuntimeError("Piper model path is not set or missing.")

        with self.coqui_lock:  # re-using lock for safety
            if self.piper_voice is not None:
                return
            self.piper_voice = PiperVoice.load(self.piper_model_path)
            print(f"Loaded Piper TTS model: {self.piper_model_path}")
